...
meth_plugin = lightbulb.Plugin("Methods")
...
@meth_plugin.command
@lightbulb.command("meth", "All the joke commands")
@lightbulb.implements(commands.PrefixCommandGroup, commands.SlashCommandGroup)
async def fun_cmd_grp(ctx: context.Context) -> None:
    await ctx.respond(
        embed=hikari.Embed(
            title="Functions Command Group",
...
@lightbulb.command("petpet", "Pet your (or someone else's) avatar.", auto_defer=True)
@lightbulb.implements(commands.PrefixSubCommand, commands.SlashSubCommand)
...
def load(bot: lightbulb.BotApp) -> None:
    bot.add_plugin(meth_plugin)


def unload(bot: lightbulb.BotApp) -> None:
    bot.remove_plugin(meth_plugin)